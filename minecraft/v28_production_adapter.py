...
Backends are injected so the core stays testable and platform-independent.
"""
...
class ProductionAdapter:
    ...
    def observe(self) -> WorldState:
        ...
        raw = self.vision.analyze(frame)
        confidence = max(0.0, min(1.0, float(raw.get("confidence", 0.0))))
        return WorldState(frame.timestamp, dict(raw.get("player", {})), tuple(raw.get("visible_blocks", ())), tuple(raw.get("entities", ())), dict(raw.get("ui", {})), confidence)
